...
class folderoperation():
    def __init__(self,fold_number,parentdir=None):
        ...
        self.base = os.path.join(self.parentdir, 'DNNevaluation')
...
class comparativeNetwork():
    ...
    def CreateDnnModel(self):
        ...
        y3 = Dense(1000,kernel_regularizer=regularizers.l2(0.0005),activation='relu')(y2)
        y_pred = Dense(CLASS_NUM, activation='softmax')(y3)
        self.dnnmodel = Model(inputs=self.model_input, outputs=y_pred)
        self.dnnmodelname = 'mfcc_dnn_1000'
        print('The dnn model with mfcc featue and [1000 1000 1000] layers is established.')
...
if __name__ == '__main__':
    ...
    for i in (0,1,2,3,4):
        ...
        print('this checking we do need training the dnn model..')
        path = os.path.join( os.getcwd(),'DNNevaluation','i=' + str(i) )
        ...
        print(90 * '=')
        print('')
        print('!Having finish the training ,let us now rebuild the dnn network and load the corresponding weights!')
        newnn = comparativeNetwork()
        newnn.CreateDnnModel()
        newnn.ModelweightsLoading(newnn.dnnmodel,path)
        print('for round_{}:'.format(str(i)))
        evaluation(report1, report2, newnn.dnnmodel, newnn.dnnmodelname,featureType='mfcc')
        gc.collect()
    en = time.time() - ev
    hour = en // 3600
