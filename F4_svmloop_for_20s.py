...
if __name__ == '__main__':
    ...
    sys.stdout = logger(filename=os.path.join(os.getcwd(),'log&&materials','Supploopsvm_20subjectsdesignevalutionresults.log'))
    # This loop is for different sizess of datasets
    topCon = {'SVM':[]}
    wholest = time.time()
    for jsonfile, flag in zip(jsonfiles, (5,10,15,20)):
        if flag != 20:
        ...
        print()
        #1. prepare the data
        for switch, fname in zip((False,True),('mfcc','stft')):
            dur = time.time()
            features, targets = skstyleDataset(file_trans_output, flag, stft_spec=switch)
            print('dataset formation time: {}s.'.format(round(time.time() - dur, 2)))
            print('This time {} spectrogram are used as features..'.format(fname))
            ...
            print('For json file {} and with the {} spectrogram, the evaluation results are: {} +/- {} %.'.format( jsonfile.split('/')[-1], fname, round(mean,2), round(stddev,2) ) )

            topCon['SVM'].append( (str(flag)+'-'+fname, searchedparas, mean, stddev) )
    ...
    print('Overall time for designing and evaluating svm models on the different scales of dataset: {}s, i.e., {} hour(s), {} minute(s), {}s'.format(en, int(hour), int(minute), seconds))
    print('Curve [dict]:')
